...
class JobList extends Component {
	...
	//fetch the static editable job listing via ajax
	componentDidMount(){
		const xhr = new XMLHttpRequest();
		let data = [];

		xhr.open('GET', `${process.env.PUBLIC_URL}/jobsList.json`, true);
		xhr.onload = function(e){
      if (xhr.readyState === 4 && xhr.status === 200){
				data = JSON.parse(xhr.response).jobs;
				...
				<div key={i} onClick={() => this.handleClick(i)} className={'panel'}>
					<div className="row">
						...
						<i
							style={{ fontSize: 25 }}
							class={
								activeSection === i ? 'fa fa-chevron-up' : 'fa fa-chevron-down'
							}
							aria-hidden="true"
						/>
					</div>
	...
	renderBulletedList = field => {
		...
		return field.map(item => {
			return (
				<p style={{ paddingLeft: 45 }} className="job-text">
					{item}
				</p>
			);
		});
			...
			return (
				<div className="expanded-container">
					<div
						...
						<h2
							style={{ fontWeight: '700', paddingLeft: 30, paddingRight: 10 }}
							className="job-text">
							Job Location:{' '}
						</h2>
					...
					<div style={{ paddingLeft: 30, paddingRight: 30 }}>
						{field.responsibilities ? (
							<h2 className="job-text">Responsibilities: </h2>
						) : null}
					...
					<div style={{ paddingLeft: 30, paddingRight: 30 }}>
						{field.personalTraits ? (
							<h2 className="job-text">Personal Traits: </h2>
						) : null}
					...
					<h4
						className="job-text"
						style={{ fontWeight: '600', paddingLeft: 30 }}>
						Please email resume/CV to jayden@vaius.group. We will get back to
						you within 24 hours to follow up.
					</h4>
				</div>
			);
		} else return;
	...
	render() {
		return (
			<div className="container">
				<div className="post-box">{this.renderList()}</div>
			</div>
		);
	}
}
